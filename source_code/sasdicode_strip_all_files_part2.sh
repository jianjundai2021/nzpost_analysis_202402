...
OUTPUT_DIR=$HOME_DIR/sas_code_folder_simple_part2/Lev1

cd $SOURCE_DIR/
for i in `ls  `
do 
  FILENAME=$(echo "$(pwd)/$i")
  python $HOME_DIR/sasdicode_strip_part2.py $FILENAME > $OUTPUT_DIR/$i
done  


SOURCE_DIR=$HOME_DIR/sas_code_folder_simple/Lev3
OUTPUT_DIR=$HOME_DIR/sas_code_folder_simple_part2/Lev3
