...
select *
from t2 
left outer join JIANJUNDAI_SAS_MERGE_JOBS   mrg 
on t2.lsf_sas_code_name = mrg.mrg_code_file_full_path
--where lsf_sas_code_name like '%DATAMART_ECL_D_CPT_MAP1.sas'
;
left outer join JIANJUNDAI_SAS_MERGE_JOBS
;
...
/saslogs/lev1/
DW_ECL_INCREMENTAL_DW_LOAD_DW_ECL_F_PCE_MAP1_2024.01.24_06.00.45.302.0.log
...
create table JIANJUNDAI_SAS_MERGE_JOBS
( 
...
create table JIANJUNDAI_SAS_MERGE_JOBS_RAW as select * from JIANJUNDAI_SAS_MERGE_JOBS
;
drop table JIANJUNDAI_SAS_MERGE_JOBS;
...
from jianjundai_lsf_log_detail
where job_start_date between to_date('01-Jul-2023', 'DD-Mon-YYYY') and to_date('19-Feb-2024', 'DD-Mon-YYYY')
group by 
     job_start_date
   , lsf_process_flow_name
order by 1, 2 
...
select 
     job_start_date as sas_process_flow_run_date
  ,  job_start_dow  as sas_process_flow_run_date_dow
  ,  count(*) 
from t1 
group by 
     job_start_date
...
select 
     job_start_date                as sas_process_flow_run_date
   , min(job_start_time)           as sas_process_flow_run_stime
   , to_char(job_start_date, 'Dy') as sas_process_flow_run_date_dow
   , lsf_process_flow_name         as sas_process_flow_name 
...
select * 
from jianjundai_lsf_log_detail
where lsf_process_flow_name is null
order by lsf_subprocess_name, job_start_time 
;
